...
import { updateCustomerInfo } from "@/actions/admin/order-mutations";
...
interface EditCustomerInfoFormModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  orderId: string;
  initialData: EditCustomerInfoInput;
  onSuccess?: () => void;
}

export function EditCustomerInfoFormModal({
  open,
  onOpenChange,
  orderId,
  initialData,
  onSuccess,
}: EditCustomerInfoFormModalProps) {
  const queryClient = useQueryClient();
  const [formError, setFormError] = useState<string | null>(null);

  // Form setup
  const form = useForm<EditCustomerInfoInput>({
  ...
  // Watch userId to detect if a registered account is attached
  const currentUserId = useWatch({ control, name: "userId" });

  // Handle user auto-fill selection
  ...
  // Unlink registered user without clearing form fields
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data: EditCustomerInfoInput) => {
      if (!orderId) throw new Error("Order ID is required.");

      const res = await updateCustomerInfo({ orderId, input: data });
  ...
  const onSubmit = (values: EditCustomerInfoInput) => {
    setFormError(null);
    mutate(values);
  };
